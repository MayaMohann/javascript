const employees = [
    ...
    { id: 4, name: 'Tom', age: 35, department: 'HR', salary: 45000, specialization: 'Java' },
   ...
   // Function to display all employees
    ...
    function findEmployeeBySpecialization(specializationEmp){
        const specializationEmployee = employees.filter(employee => employee.specialization === specializationEmp)
        const specializedEmployee = specializationEmployee.map((employee, index) =>
            `<p>${employee.id}: ${employee.name}: ${employee.name} - ${employee.department} - $${employee.salary} - ${employee.specialization}</p>`).join('');
        document.getElementById('employeesDetails').innerHTML = specializedEmployee;
    }